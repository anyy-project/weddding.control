...
function initEventListeners() {
    ...
    // Contact Picker - Check support on load
    ...
    try {
        ...
        if (contacts && contacts.length > 0) {
            ...
            if (selectedContacts.length > 0) {
                ...
                showToast(`✅ ${selectedContacts.length} kontak terpilih`, 'success');
            } else {
        ...
        if (error.name === 'AbortError') {
            ...
            showToast('❌ Akses kontak ditolak. Izinkan akses kontak di browser settings.', 'error');
        } else if (error.name === 'NotSupportedError') {
            showToast('❌ Contact Picker tidak support di browser ini', 'error');
        } else {
